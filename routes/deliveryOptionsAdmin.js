...
function parsePriceToCents(input) {
  ...
  const str = String(input).trim().replace(/[$,R\s]/gi, ""); // remove currency symbols/spaces
  if (str === "") return null;
  const n = Number(str);
...
/* ------------------------------------------------------------------
 * 🎛️ Common locals (theme + nonce + breadcrumbs)
 * ------------------------------------------------------------------ */
function viewLocals(req, more = {}) {
...
router.get("/admin/delivery-options", requireOrdersAdmin, async (req, res) => {
  try {
    ...
    const [items, total] = await Promise.all([
      DeliveryOption.find(where).sort({ [sortBy]: order }).skip(skip).limit(limit).lean(),
...
router.post(
  ...
  [
    ...
    body("minDays").optional({ checkFalsy: true }).isInt({ min: 0, max: 60 }).withMessage("minDays 0-60"),
    body("maxDays").optional({ checkFalsy: true }).isInt({ min: 0, max: 60 }).withMessage("maxDays 0-60"),
  ],
  async (req, res) => {
    const theme = req.session.theme || "light";
    const themeCss = theme === "dark" ? "/css/dark.css" : "/css/light.css";
