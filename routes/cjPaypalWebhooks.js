...
function captureOrderId(event) {
  return safeString(event?.resource?.supplementary_data?.related_ids?.order_id, 200);
}
...
async function findCjOrder(event) {
  const paypalOrderId = captureOrderId(event) || checkoutOrderId(event);
...
router.post('/paypal', async (req, res) => {
  ...
  try {
    ...
    /*
     * Track whether this webhook genuinely changed the order
     * to cancelled. A paid completed order must not receive a
     * false cancellation email when PayPal sends a late
     * CHECKOUT.ORDER.CANCELLED event.
     */
    let orderWasCancelled = false;

    const incomingCaptureId = captureId(event);

    const incomingStatus = captureStatus(event) || eventType.replace('PAYMENT.CAPTURE.', '');
    ...
    if (eventType.startsWith('PAYMENT.CAPTURE.')) {
      order.paypal.captureStatus = incomingStatus;
    ...
    if (eventType === 'PAYMENT.CAPTURE.REFUNDED') {
      ...
      order.supplierOrder.createStatus = 'NOT_CREATED';
      order.lastPaymentErrorCode = 'PAYPAL_CAPTURE_REFUNDED';
      order.lastPaymentErrorMessage = 'PayPal reported that the CJ payment capture was refunded.';
    }
    ...
    if (eventType === 'CHECKOUT.ORDER.CANCELLED') {
      ...
      if (String(order.paymentStatus || '').toUpperCase() !== 'COMPLETED') {
        ...
        order.lastPaymentErrorCode = 'PAYPAL_ORDER_CANCELLED';
        order.lastPaymentErrorMessage = 'PayPal reported that the checkout order was cancelled.';
    ...
    if (orderWasCancelled) {
      await sendCjOrderEventEmailsSafely(order, 'CANCELLED', {
        source: 'cj-paypal-webhook',
      });
    }
