...
if (!BUCKET) {
  console.warn('⚠️ AWS_BUCKET_NAME missing — shop main banner uploads will fail.');
}
  ...
  async (req, res) => {
    try {
      ...
      if (!payload.productCustomId) {
        ...
        return res.redirect('/admin/shop-main-banner/edit');
      }
      ...
      console.error('❌ save shop main banner error:', err);
      req.flash('error', err.message || 'Failed to save shop main banner.');
      return res.redirect('/admin/shop-main-banner');
    }
  }
);
...
router.use((err, req, res, _next) => {
  console.error('❌ adminShopMainBanner route error:', err.message);
