// routes/adminBestsellerBottomBanners.js
'use strict';
...
if (!BUCKET) {
  console.warn('⚠️ AWS_BUCKET_NAME missing — bestseller bottom banner uploads will fail.');
}
  ...
  async (req, res) => {
    try {
      ...
      if (!payload.productCustomId) {
        ...
        return res.redirect(`/admin/bestseller-bottom-banners/${slot}/edit`);
      }
      ...
      if (!banner) {
        ...
        banner.overlayStyle = payload.overlayStyle;
      ...
      req.flash(
        'success',
        `${slot === 'left' ? 'Left' : 'Right'} bestseller bottom banner saved successfully.`
      );
      return res.redirect('/admin/bestseller-bottom-banners');
    } catch (err) {
      console.error('❌ save bestseller bottom banner error:', err);
      req.flash('error', err.message || 'Failed to save bestseller bottom banner.');
      return res.redirect('/admin/bestseller-bottom-banners');
    }
  }
);
...
router.use((err, req, res, _next) => {
  console.error('❌ adminBestsellerBottomBanners route error:', err.message);
