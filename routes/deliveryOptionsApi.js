...
router.get("/api/delivery-options", async (req, res) => {
  try {
    ...
    const minDays = Number.isFinite(Number(req.query.minDays)) ? Number(req.query.minDays) : undefined;
    const maxDays = Number.isFinite(Number(req.query.maxDays)) ? Number(req.query.maxDays) : undefined;
