...
function bestsellerCardSnapshot(card) {
  if (!card) return null;

  return {
    slot: card.slot || '',
    productCustomId: card.productCustomId || '',
...
router.post(
  ...
  async (req, res) => {
    try {
      ...
      const before = bestsellerCardSnapshot(card);
      const isCreate = !card;
      ...
      await logAdminAction(req, {
        action: isCreate ? 'store.bestseller_card.create' : 'store.bestseller_card.update',
        entityType: 'bestseller_card',
        entityId: String(card._id),
        status: 'success',
        before,
        after: bestsellerCardSnapshot(card),
        meta: {
          section: 'bestseller_cards',
          slot,
...
router.get(
  ...
  async (req, res) => {
    try {
      ...
      await logAdminAction(req, {
        action: card.active ? 'store.bestseller_card.activate' : 'store.bestseller_card.deactivate',
        entityType: 'bestseller_card',
        entityId: String(card._id),
        status: 'success',
        before,
