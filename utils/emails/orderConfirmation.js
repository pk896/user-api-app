...
function formatMoney(value, currency) {
  ...
  const ccy = String(currency || 'USD').trim().toUpperCase() || 'USD';
...
function buildOrderConfirmationEmail(order, baseUrl) {
  const o = order || {};
  const safeBaseUrl = sanitizeBaseUrl(baseUrl || process.env.PUBLIC_BASE_URL || process.env.APP_URL || process.env.FRONTEND_URL || '');

  const orderId = String(o.orderId || '').trim();
  const customerEmail = String(o.payer?.email || o.shipping?.email || '').trim();

  const trackingUrl = safeBaseUrl
    ? `${safeBaseUrl}/store/order-tracking`
    : '/store/order-tracking';

  const amountText = formatMoney(o.amount?.value || 0, o.amount?.currency || process.env.BASE_CURRENCY || 'USD');
