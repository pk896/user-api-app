...
function firstDocumentError(documents) {
  const errors = Array.isArray(documents?.errors)
    ? documents.errors
    : [];
...
async function saveCourierGuyShipmentToOrder(
  order,
  result,
) {
  if (!order) {
    const error = new Error(
      'Order is required when saving a Courier Guy shipment.',
    );

    error.code =
      'COURIER_GUY_ORDER_REQUIRED';
  ...
  order.courierGuy.stickerUrl =
    shipment.stickerUrl ||
    order.courierGuy.stickerUrl ||
    '';

  order.courierGuy.trackingUrl =
  ...
  order.courierGuy.lastTrackingSyncAt =
    new Date();
  ...
  if (result?.collectionAddress) {
    order.courierGuy.collectionAddressSnapshot =
      result.collectionAddress;
  }

  if (result?.deliveryAddress) {
    order.courierGuy.deliveryAddressSnapshot =
  ...
  if (result?.trackingResponse) {
    order.courierGuy.trackingResponse =
  ...
  if (result?.documents) {
    ...
    if (documents.waybillUrl) {
      order.courierGuy.waybillUrl =
        documents.waybillUrl;
    }

    if (documents.stickerUrl) {
      order.courierGuy.stickerUrl =
        documents.stickerUrl;
    }

    const stickerParcels =
      Array.isArray(documents.stickerParcels)
        ? documents.stickerParcels
        : [];

    const stickerZpl = String(
      documents.stickerZpl || '',
    );

    if (stickerZpl.trim()) {
      order.courierGuy.stickerFormat =
        String(
          documents.format || 'zpl',
        )
          .trim()
          .toLowerCase();

      order.courierGuy.stickerParcels =
        stickerParcels;

      order.courierGuy.stickerZpl =
        stickerZpl;

      order.courierGuy.stickerGeneratedAt =
        new Date();
  ...
  if (Array.isArray(shipment.events)) {
    order.shippingTracking.liveEvents =
      shipment.events;
  }
  ...
  order.shippingTracking.lastTrackingUpdate =
    Number.isNaN(
      providerLastUpdate.getTime(),
    )
      ? new Date()
      : providerLastUpdate;

  order.shippingTracking.lastUpdate =
    new Date();
