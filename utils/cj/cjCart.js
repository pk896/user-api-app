...
function calculateCjCartSummary(items = []) {
  ...
  const summary = list.reduce(
    (accumulator, item) => {
      ...
      accumulator.subtotalExVat += unitPriceExVat * quantity;
      accumulator.totalIncVat += unitPriceIncVat * quantity;
  ...
  summary.subtotalExVat = round2(summary.subtotalExVat);
  summary.totalIncVat = round2(summary.totalIncVat);
  summary.vatAmount = round2(
    summary.totalIncVat - summary.subtotalExVat,
  );
...
function getVariantById(product, cjVariantId) {
  ...
  return (
    product.variants.find(
      (variant) =>
        String(variant?.cjVariantId || '').trim() === wantedId,
    ) || null
  );
}
...
async function loadSellableCjProductAndVariant({
  cjProductId,
  cjVariantId,
}) {
  ...
  if (!product) {
    const error = new Error(
      'This CJ product is not currently available for sale.',
    );
  ...
  if (!variant || variant.isEnabled !== true) {
    const error = new Error(
      'This CJ product variant is not currently available.',
    );
    error.code = 'CJ_VARIANT_NOT_AVAILABLE';
    throw error;
  }

  const priceExVat = Number(
  ...
  if (
    ...
    const error = new Error(
      'This CJ variant does not have a valid Kasyora selling price.',
    );
...
function buildCjCartItem({
  ...
  return {
    ...
    dimensionsMm: {
      length:
        Number.isFinite(Number(variant?.dimensionsMm?.length))
          ? Number(variant.dimensionsMm.length)
          : null,

      width:
        Number.isFinite(Number(variant?.dimensionsMm?.width))
          ? Number(variant.dimensionsMm.width)
          : null,

      height:
        Number.isFinite(Number(variant?.dimensionsMm?.height))
...
function findCjCartItemIndex(items, cjVariantId) {
  ...
  return (Array.isArray(items) ? items : []).findIndex(
    (item) =>
      String(item?.cjVariantId || '').trim() === wantedId,
  );
}

async function addCjItem(req, {
  cjProductId,
  cjVariantId,
  quantity,
}) {
   ...
   */
  if (
    loaded.inventory.inventoryKnown &&
    loaded.inventory.totalInventory < safeQuantity
  ) {
    const error = new Error(
  ...
  const existingIndex = findCjCartItemIndex(
    cart.items,
    loaded.variant.cjVariantId,
  );
  ...
  if (existingIndex >= 0) {
    ...
    if (
      loaded.inventory.inventoryKnown &&
      loaded.inventory.totalInventory < nextQuantity
    ) {
      const error = new Error(
        `Only ${loaded.inventory.totalInventory} unit(s) are currently available.`,
      );
...
async function updateCjItemQuantity(req, {
  cjVariantId,
  quantity,
}) {
  const cart = ensureCjCart(req);
  ...
  const index = findCjCartItemIndex(
    cart.items,
    cleanVariantId,
  );
